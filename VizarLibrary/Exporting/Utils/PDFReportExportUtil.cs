...
public static class PDFReportExportUtil
{
		...
		Dictionary<string, string> customSummaryFields = null)
	{
		...
		try
		{
			using (PdfDocument pdfDocument = new())
			{
				...
				// Add data grid
				PdfLayoutResult gridResult = AddDataGrid(page, pdfDocument, data, effectiveColumnOrder, columnSettings, currentY, useBuiltInStyle);
		...
		Dictionary<string, string> headerMetadata = null)
	{
		...
		if (headerMetadata != null && headerMetadata.Any())
		{
