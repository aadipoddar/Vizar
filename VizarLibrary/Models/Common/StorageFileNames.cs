namespace VizarLibrary.Models.Common;

public static class StorageFileNames
{
	public static string UserDataFileName => "user_data.json";
	public static string UserDeviceIdDataFileName => "user_device_id_data.json";

	public static string FinancialAccountingDataFileName => "financial_accounting_data.json";
	public static string FinancialAccountingCartDataFileName => "financial_accounting_cart_data.json";

	public static string PurchaseDataFileName => "purchase_data.json";
	public static string PurchaseCartDataFileName => "purchase_cart_data.json";
	public static string PurchaseReturnDataFileName => "purchase_return_data.json";
	public static string PurchaseReturnCartDataFileName => "purchase_return_cart_data.json";
	public static string ItemIssueDataFileName => "item_issue_data.json";
	public static string ItemIssueCartDataFileName => "item_issue_cart_data.json";

	public static string ItemStockAdjustmentCartDataFileName => "item_stock_adjustment_cart_data.json";
}