...
BEGIN
	...
	WITH StockAggregates AS (
		SELECT 
			...
			SUM(CASE WHEN TransactionDateTime < @StartDate THEN Quantity ELSE 0 END) AS OpeningStock,
			
			-- Purchase Stock: sum of positive quantities in date range
			SUM(CASE WHEN TransactionDateTime >= @StartDate AND TransactionDateTime <= @EndDate AND Quantity > 0 
			...
			SUM(CASE WHEN TransactionDateTime >= @StartDate AND TransactionDateTime <= @EndDate 
				THEN Quantity ELSE 0 END) AS MonthlyStock,
			
			-- Closing Stock: sum of all quantities up to ToDate
			SUM(CASE WHEN TransactionDateTime <= @EndDate THEN Quantity ELSE 0 END) AS ClosingStock
