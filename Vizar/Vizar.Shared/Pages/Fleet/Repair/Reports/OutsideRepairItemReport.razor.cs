...
public partial class OutsideRepairItemReport : IAsyncDisposable
{
	...
	private List<OutsideRepairItemOverviewModel> _transactionOverviews = [];

	private SfGrid<OutsideRepairItemOverviewModel> _sfGrid;
	...
	private async Task LoadTransactionOverviews()
	{
		...
		try
		{
			...
			_transactionOverviews = await CommonData.LoadTableDataByDate<OutsideRepairItemOverviewModel>(
				ViewNames.OutsideRepairItemOverview,
			...
			if (_showSummary)
				_transactionOverviews = [.. _transactionOverviews
					.GroupBy(t => t.Job)
					.Select(g => new OutsideRepairItemOverviewModel
					{
						Job = g.Key,
	...
	private async Task ExportExcel()
	{
		...
		try
		{
			...
			var (stream, fileName) = await OutsideRepairReportExport.ExportItemReport(
	...
	private async Task DownloadExcelInvoice(int transactionId)
	{
		...
		try
		{
			...
			var (pdfStream, fileName) = await OutsideRepairInvoiceExport.ExportInvoice(transactionId, InvoiceExportType.Excel);
			await SaveAndViewService.SaveAndView(fileName, pdfStream);
