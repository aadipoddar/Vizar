...
public partial class OutsideRepairReport : IAsyncDisposable
{
	...
	private List<OutsideRepairOverviewModel> _transactionOverviews = [];

	private SfGrid<OutsideRepairOverviewModel> _sfGrid;
	...
	private async Task LoadData()
	{
		_hotKeysContext = HotKeys.CreateContext()
			...
			.Add(ModCode.Ctrl, Code.I, NavigateToItemReport, "Open job report", Exclude.None)
	...
	private async Task LoadTransactionOverviews()
	{
		...
		try
		{
			...
			_transactionOverviews = await CommonData.LoadTableDataByDate<OutsideRepairOverviewModel>(
				ViewNames.OutsideRepairOverview,
			...
			if (_showSummary)
				_transactionOverviews = [.. _transactionOverviews
					...
					.Select(g => new OutsideRepairOverviewModel
					{
						VehicleCode = g.Key,
	...
	private async Task ExportExcel()
	{
		...
		try
		{
			...
			var (stream, fileName) = await OutsideRepairReportExport.ExportReport(
	...
	private async Task ViewTransaction(int transactionId)
	{
		try
		{
			if (FormFactor.GetFormFactor() == "Web")
				await JSRuntime.InvokeVoidAsync("open", $"{PageRouteNames.OutsideRepair}/{transactionId}", "_blank");
			else
				NavigationManager.NavigateTo($"{PageRouteNames.OutsideRepair}/{transactionId}");
		}
		catch (Exception ex)
		{
	...
	private async Task DownloadPdfInvoice(int transactionId)
	{
		...
		try
		{
			...
			var (pdfStream, fileName) = await OutsideRepairInvoiceExport.ExportInvoice(transactionId, InvoiceExportType.PDF);
	...
	private async Task DownloadExcelInvoice(int transactionId)
	{
		...
		try
		{
			...
			var (excelStream, fileName) = await OutsideRepairInvoiceExport.ExportInvoice(transactionId, InvoiceExportType.Excel);
	...
	{
		var transaction = await CommonData.LoadTableDataById<OutsideRepairModel>(TableNames.OutsideRepair, deleteTransactionId);
		...
		transaction.LastModifiedBy = _user.Id;
		transaction.LastModifiedAt = await CommonData.LoadCurrentDateTime();
		transaction.LastModifiedFromPlatform = FormFactor.GetFormFactor() + FormFactor.GetPlatform();

		await OutsideRepairData.DeleteTransaction(transaction);
	}
	...
	{
		var transaction = await CommonData.LoadTableDataById<OutsideRepairModel>(TableNames.OutsideRepair, recoverTransactionId);
		if (transaction is null)
		{
			await _toastNotification.ShowAsync("Error", "Transaction not found.", ToastType.Error);
			return;
		...
		await OutsideRepairData.RecoverTransaction(transaction);
	}
	#endregion
	...
	{
		if (FormFactor.GetFormFactor() == "Web")
			await JSRuntime.InvokeVoidAsync("open", PageRouteNames.OutsideRepair, "_blank");
		else
			NavigationManager.NavigateTo(PageRouteNames.OutsideRepair);
	}
