...
public partial class ServiceReport : IAsyncDisposable
{
    ...
    private List<ServiceOverviewModel> _transactionOverviews = [];

    private SfGrid<ServiceOverviewModel> _sfGrid;
    ...
    private async Task LoadTransactionOverviews()
    {
        ...
        try
        {
            ...
            _transactionOverviews = await CommonData.LoadTableDataByDate<ServiceOverviewModel>(
                ViewNames.ServiceOverview,
            ...
            if (_showSummary)
                _transactionOverviews = [.. _transactionOverviews
                    ...
                    .Select(g => new ServiceOverviewModel
                    {
                        GarageName = g.Key,
    ...
    {
        var service = await CommonData.LoadTableDataById<ServiceModel>(TableNames.Service, deleteTransactionId);
        if (service is null)
        {
        ...
        service.LastModifiedBy = _user.Id;
        service.LastModifiedAt = await CommonData.LoadCurrentDateTime();
        service.LastModifiedFromPlatform = FormFactor.GetFormFactor() + FormFactor.GetPlatform();

        await ServiceData.DeleteTransaction(service);
    }
    ...
    {
        var service = await CommonData.LoadTableDataById<ServiceModel>(TableNames.Service, recoverTransactionId);
        if (service is null)
        {
            await _toastNotification.ShowAsync("Error", "Transaction not found.", ToastType.Error);
            return;
        ...
        await ServiceData.RecoverTransaction(service);
    }
    #endregion
