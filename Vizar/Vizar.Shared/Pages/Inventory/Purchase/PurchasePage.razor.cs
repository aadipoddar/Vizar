...
public partial class PurchasePage : IAsyncDisposable
{
	...
	private async Task<bool> ValidateForm()
	{
		...
		_purchase.DocumentUrl = _purchase.DocumentUrl?.Trim();
	...
	private async Task DownloadExistingDocument()
	{
		try
		{
			...
			var (fileStream, contentType) = await BlobStorageAccess.DownloadFileFromBlobStorage(_purchase.DocumentUrl);
			var fileName = _purchase.DocumentUrl.Split('/').Last();
	...
	private async Task DownloadPdfInvoice()
	{
		...
		try
		{
			...
			var (pdfStream, fileName) = await PurchaseData.GenerateAndDownloadInvoice(Id.Value);
			await SaveAndViewService.SaveAndView(fileName, pdfStream);
	...
	private async Task DownloadExcelInvoice()
	{
		...
		try
		{
			...
			var (excelStream, fileName) = await PurchaseData.GenerateAndDownloadExcelInvoice(Id.Value);
