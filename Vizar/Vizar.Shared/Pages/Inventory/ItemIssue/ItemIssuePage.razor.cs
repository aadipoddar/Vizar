...
public partial class ItemIssuePage : IAsyncDisposable
{
	...
	private async Task SaveTransaction()
	{
		...
		try
		{
			...
			var (pdfStream, fileName) = await ItemIssueData.GenerateAndDownloadInvoice(_itemIssue.Id);
	...
	{
		if (FormFactor.GetFormFactor() == "Web")
			await JSRuntime.InvokeVoidAsync("open", PageRouteNames.ReportItemIssueItem, "_blank");
		else
			NavigationManager.NavigateTo(PageRouteNames.ReportItemIssueItem);
	}
	...
	private async Task DownloadPdfInvoice()
	{
		...
		try
		{
			...
			var (pdfStream, fileName) = await ItemIssueData.GenerateAndDownloadInvoice(Id.Value);
			await SaveAndViewService.SaveAndView(fileName, pdfStream);
	...
	private async Task DownloadExcelInvoice()
	{
		...
		try
		{
			...
			var (excelStream, fileName) = await ItemIssueData.GenerateAndDownloadExcelInvoice(Id.Value);
